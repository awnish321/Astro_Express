...
import com.astroexpress.user.ui.activity.AstrologerListActivity;
...
public class LoadAstrologerCallListAdapter extends RecyclerView.Adapter<LoadAstrologerCallListAdapter.ImageViewHolder> {
    private Context context;
    private List<AstrologerResponseModel.Result> resultList;

    public LoadAstrologerCallListAdapter(Context context, List<AstrologerResponseModel.Result> resultList) {
    ...
    public void onBindViewHolder(@NonNull ImageViewHolder holder, int position) {
        ...
        if (itemData.isOnlineForCall())
        {
            holder.binding.imgBtnCall.setVisibility(View.VISIBLE);
            if (itemData.getBusyOnCall()) {
                ...
                holder.binding.imgBtnCall.setEnabled(false);

            } else {
                holder.binding.txtAstroStatus.setText("Online");
                holder.binding.txtAstroStatus.setBackgroundResource(R.color.green);
                holder.binding.imgBtnCall.setEnabled(true);
            }

        } else {
            holder.binding.imgBtnCall.setVisibility(View.GONE);
        ...
        holder.binding.imgBtnCall.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (AllStaticFields.userData != null) {
                    ...
                    if (AllStaticFields.sessionId == null) {
                        ...
                        context.startActivity(new Intent(context, UpdateRegistrationFormActivity.class).setAction(ASTROLOGERS_FOR_CALL)
                                .putExtra("AstrologerId", itemData.getAstrologerId())
                                .putExtra("AstrologerChargePerMinute", itemData.getChargePerMinute())
        ...
        if (!itemData.getWaitTime().toString().equals("0") &&  itemData.isOnlineForCall()) {
